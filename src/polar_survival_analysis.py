"""
극좌표 변환 데이터용 Cox Proportional Hazards 생존 분석 모듈
"""
import pandas as pd
import numpy as np
...
class PolarSurvivalAnalyzer(SurvivalAnalyzer):
    """극좌표 변환 데이터용 생존 분석 클래스"""
    ...
    def run_analysis_for_target(self, target_var: str):
        ...
        print(f"\n{'='*70}")
        print(f"🔬 {target_var.upper()} 극좌표 생존 분석 시작")
        print(f"{'='*70}\n")
        ...
        # HTML 리포트 생성
        report_file = self.output_dir / f"{target_var}_survival_analysis_polar_report.html"
...
def main():
    """메인 실행 함수"""
    analyzer = PolarSurvivalAnalyzer()

    target_vars = ['anxiety', 'depression', 'stress']
    all_results = {}

    print(f"\n{'='*70}")
    print(f"🚀 극좌표 생존 분석 시작 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
