"""
원통좌표 변환 모듈
데이터를 원통좌표 시스템(Cylindrical Coordinate System)으로 변환하여 새로운 특성 생성
"""
import pandas as pd
import numpy as np
    ...
    def transform_dataframe(self, df: pd.DataFrame, exclude_cols: List[str] = None) -> pd.DataFrame:
        ...
        for i in range(0, len(numeric_cols), 3):
            if i + 2 < len(numeric_cols):
                ...
                rho_name = f"rho_{pair_name}"
                phi_name = f"phi_{pair_name}"

                result_df[rho_name] = rho
                result_df[phi_name] = phi

                self.cylindrical_features.extend([rho_name, phi_name])
        ...
        print(f"✅ 원통좌표 변환 완료: {len(self.feature_triples)}개 트리플 -> {len(self.cylindrical_features)}개 원통좌표 특성")

        return result_df

    def transform_splits(self, data_splits_dir: str = "data_splits",
                        output_dir: str = "data_splits_cylinder",
                        target_vars: List[str] = None):
        """
        train/val/test 분할 데이터를 모두 원통좌표로 변환
        ...
        for target in target_vars:
            ...
            for split in ['train', 'val', 'test']:
                ...
                output_file = output_path / f"{target}_{split}_cylinder.csv"
                ...
                # 원통좌표 변환
                df_cyl = self.transform_dataframe(df)

                # 저장
                df_cyl.to_csv(output_file, index=False)
                print(f"  ✅ 저장 완료: {output_file.name} ({len(df_cyl):,}행 x {len(df_cyl.columns)}열)")

        print(f"\n{'='*60}")
        print(f"✅ 모든 데이터 원통좌표 변환 완료")
        print(f"{'='*60}\n")
        ...
        summary_df.to_csv(output_path / "cylindrical_transformation_summary.csv", index=False)
        print(f"📊 변환 요약 저장: cylindrical_transformation_summary.csv\n")
...
def main():
    """메인 실행 함수"""
    transformer = CylindricalTransformer()
