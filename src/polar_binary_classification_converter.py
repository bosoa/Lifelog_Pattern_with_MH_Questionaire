...
class PolarBinaryClassificationConverter:
    """극좌표 변환 데이터를 이진 분류 형식으로 변환하는 클래스"""

    def __init__(self, input_dir: str = "data_splits_polar",
                 output_dir: str = "hierarchical_data_polar"):
    ...
    def create_binary_classification_data(self, target_var: str,
        ...
        """
        print(f"\n{'='*60}")
        print(f"📊 {target_var.upper()} 극좌표 이진 분류 데이터 생성")
        print(f"{'='*60}\n")

        # Train 데이터 로드
        train_file = self.input_dir / f"{target_var}_train_polar.csv"
        ...
        for split in ['val', 'test']:
            split_file = self.input_dir / f"{target_var}_{split}_polar.csv"
        ...
        # 저장
        output_file = self.output_dir / f"{target_var}_binary_classification_polar.csv"
    ...
    def create_all_binary_classification_data(self):
        ...
        summary_file = self.output_dir / "binary_classification_summary_polar.csv"
        summary_df.to_csv(summary_file, index=False)

        print(f"\n{'='*60}")
        print(f"✅ 극좌표 이진 분류 데이터 생성 완료")
        print(f"{'='*60}\n")
