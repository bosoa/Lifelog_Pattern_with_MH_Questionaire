...
class PolarModelComparison(ModelComparison):
    """극좌표 변환 데이터용 모델 비교 클래스"""
    ...
    def run_comparison_for_target(self, target_var: str):
        ...
        print(f"\n{'='*70}")
        print(f"🔬 {target_var.upper()} 극좌표 모델 비교 분석")
        print(f"{'='*70}\n")

        # Train 데이터 로드
        train_file = self.input_dir / f"{target_var}_train_polar.csv"
        val_file = self.input_dir / f"{target_var}_val_polar.csv"
        test_file = self.input_dir / f"{target_var}_test_polar.csv"
        ...
        # 저장
        summary_file = self.output_dir / f"{target_var}_model_comparison_polar.csv"
...
def main():
    """메인 실행 함수"""
    comparer = PolarModelComparison()

    target_vars = ['anxiety', 'depression', 'stress']
    all_results = {}

    print(f"\n{'='*70}")
    print(f"🚀 극좌표 모델 비교 분석 시작 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
