"""
표준화 기반 구면좌표 변환 데이터용 Cox Proportional Hazards 생존 분석 모듈
"""
import pandas as pd
import numpy as np
...
class StandardizedSphericalSurvivalAnalyzer(SurvivalAnalyzer):
    """표준화 기반 구면좌표 변환 데이터용 생존 분석 클래스"""

    def __init__(self, data_splits_dir: str = "data_splits_standardized_sphere",
                 output_dir: str = "model_results_standardized_sphere"):
        super().__init__()
    ...
    def prepare_data_for_analysis(self, target_var: str):
        """표준화 구면좌표 데이터를 생존 분석용으로 준비"""
        train_file = self.data_splits_dir / f"{target_var}_train_standardized_spherical.csv"
    ...
    def run_analysis_for_target(self, target_var: str):
        ...
        print(f"\n{'='*70}")
        print(f"🔬 {target_var.upper()} 표준화 구면좌표 생존 분석 시작")
        print(f"{'='*70}\n")
        ...
        report_file = self.output_dir / f"{target_var}_survival_analysis_standardized_spherical_report.html"
...
def main():
    """메인 실행 함수"""
    analyzer = StandardizedSphericalSurvivalAnalyzer()

    target_vars = ['anxiety', 'depression', 'stress']
    all_results = {}

    print(f"\n{'='*70}")
    print(f"🚀 표준화 기반 구면좌표 생존 분석 시작 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*70}\n")
